...
initial_fuel = [int(element) for element in input().split()]
additional_consumption_index = deque(int(element) for element in input().split())
amount_of_fuel_needed = deque(int(element) for element in input().split())
...
if max_altitude_count == altitude_count:
    print(f"John has reached all altitudes and managed to reach the top!")